...
public interface PersonControllerDocs {
        // Criar usuário
        @Operation(summary = "Adicionando um novo Usuário", description = "Adicionando um novo Usuário no Sistema", tags = {
                        "People" }, responses = {
                                        @ApiResponse(description = "Success", responseCode = "200", content = @Content(schema = @Schema(implementation = PersonRequest.class))),
        ...
        @Operation(summary = "Listando todos os Usuários", description = "Listando todos os Usuários", tags = {
                        "People" }, responses = {
                                        @ApiResponse(description = "Success", responseCode = "200", content = {
                                                        @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, array = @ArraySchema(schema = @Schema(implementation = PersonRequest.class)))
                                        }),
        ...
        ResponseEntity<Void> update(@PathVariable Long id, @RequestBody PersonRequest request);

        // Deletar usuário (desabilitar)
        @Operation(summary = "Deletando o Usuário", description = "Deletando um Usuário Específico", tags = {
                        "People" }, responses = {
                                        @ApiResponse(description = "Success", responseCode = "200", content = @Content(schema = @Schema(implementation = PersonRequest.class))),
                                        @ApiResponse(description = "No Content", responseCode = "204", content = @Content),
                                        @ApiResponse(description = "Bad Request", responseCode = "400", content = @Content),
